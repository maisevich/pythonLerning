...
= = ["Сегодня — идеальный день для темного лагера"]
= = ["Но помните, что даже в этом случае нужно не забывать про светлный эль"]
...
def callback_worker(call):
    ...
    if call.data == "marochnoe_white":
        ...
        msg = '*Марочное светлое*=Пиво светлое пастеризованное. Бугель-бутылка 0,5 л, стекло. Состав: питьевая вода, солод пивоваренный ' \
              'ячменный светлый, хмель. Экстрактивность начального сусла: 11 %. ' \
              'Алкоголь: 4,5 % об.'
        bot.send_message(call.message.chat.id , msg)
    if call.data == "marochnoe_black":
        msg =  '*Марочное темное*=Пиво тёмное пастеризованное. Бугель-бутылка 0,5 л., стекло. ' \
    ...
    if call.data == "remeslennoe_white":
        msg = '*Ремесленное* светлое=Пиво светлое нефильтрованное неосветленное непастеризованное. Бугель-бутылка 1 л., стекло. ' \
              'Состав: питьевая вода, солод пивоваренный ячменный светлый, хмель. ' \
              'Экстрактивность начального сусла 11%. Алкоголь: 4,5 % об.'
    ...
    if call.data == "remeslennoe_black":
        msg = '*Ремесленное* темное=Пиво темное нефильтрованное неосветленное непастеризованное. Бугель-бутылка 1 л., стекло. ' \
        ...
        bot.send_message(call.message.chat.id , msg)
    if call.data == "kraft":
        msg = '*Крафтовое*=Пиво светлое пастеризованное. Бугель-бутылка 0,75 л., стекло. ' \
              'Состав: питьевая вода, солод пивоваренный ячменный светлый, хмель, хмелепродукты. ' \
              'Экстрактивность начального сусла: 11 % ' \
              'Алкоголь: 4,5 % об.'
        bot.send_message(call.message.chat.id , msg)
    if call.data == "zigul":
        msg = '*Жигулевское*=Пиво светлое пастеризованное. ПЭТ бутылка 1,5 л. Состав: питьевая вода, солод пивоваренный ячменный светлый, ' \
              'ячмень, хмель. Экстрактивность начального сусла: 11% Алкоголь: 4,5 %. '
        bot.send_message(call.message.chat.id, msg)
    if call.data == "dobroe_white":
        msg = '*Афанасий Доброе Светлое*=Легкое пиво с чистым солодовым вкусом и приятной хмельной горчинкой. ' \
    ...
    if call.data == "dobroe_strong":
        msg = '*Афанасий Доброе Крепкое*=Пиво светлое пастеризованное. ПЭТ бутылка 1,5 л. Состав: питьевая вода, солод пивоваренный ячменный ' \
              'светлый, хмель. Экстрактивность начального сусла: 16%. Алкоголь: 6,9 %. '
        bot.send_message(call.message.chat.id, msg)
    if call.data == "exper":
        msg = '*Экспериментальное*=Легкое пиво высокой плотности, темно-каштанового цвета, ' \
    ...
    if call.data == "ecobeer":
        msg = '*ЭкоBeer*=Пиво светлое пастеризованное. ПЭТ Бутылка 0,5 л. ' \
              'Состав: питьевая вода, солод пивоваренный ячменный светлый, ячмень, хмель.' \
    ...
    if call.data == "tverskoe_big":
        msg = '*Тверское* 1,9 л.=Пиво светлое фильтрованное пастеризованное. Бутылка 1,9 л, ПЭТ ' \
              'Состав: питьевая вода, солод пивоваренный ячменный светлый, ячмень, хмель. ' \
              'Экстрактивность начального сусла: 10 % ' \
              'Алкоголь: 4,1 % об. Срок годности – 180 суток '
        bot.send_message(call.message.chat.id , msg)
